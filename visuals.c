...
vec4_t gameBackColor = { 0.0, 0.0, 0.2, 0.75 };
vec4_t gameBorderColor = { 0.255f, 0.255f, 0.255f, 0.80f };
...
if (seth.value[VAR_CHAMS] == CHAM_PORTAL) {
	...
	if (IS_PLAYER_ENEMY(refEnt->entityNum)) {
	 ...
	 if (!IS_PLAYER_ENEMY(refEnt->entityNum)) {
                if (visible) {
                        refEnt->customShader = eth.friendVisibleShader;
                        refEnt->shaderRGBA[0] = seth.value[VAR_FRIEND_VISIBLE_RED];
                        refEnt->shaderRGBA[1] = seth.value[VAR_FRIEND_VISIBLE_GREEN];
                        refEnt->shaderRGBA[2] = seth.value[VAR_FRIEND_VISIBLE_BLUE];
			refEnt->shaderRGBA[3] = 200;
                } else {
                        refEnt->customShader = eth.friendNotVisibleShader;
                        refEnt->shaderRGBA[0] = seth.value[VAR_FRIEND_NOT_VISIBLE_RED];
                        refEnt->shaderRGBA[1] = seth.value[VAR_FRIEND_NOT_VISIBLE_GREEN];
                        refEnt->shaderRGBA[2] = seth.value[VAR_FRIEND_NOT_VISIBLE_BLUE];
			refEnt->shaderRGBA[3] = 200;
                }
                ...
                if (visible) {
                        ...
                        refEnt->shaderRGBA[0] = seth.value[VAR_ENEMY_VISIBLE_RED];
                        refEnt->shaderRGBA[1] = seth.value[VAR_ENEMY_VISIBLE_GREEN];
                        refEnt->shaderRGBA[2] = seth.value[VAR_ENEMY_VISIBLE_BLUE];
			refEnt->shaderRGBA[3] = 200;
                } else {
                        refEnt->customShader = eth.enemyNotVisibleShader;
                        refEnt->shaderRGBA[0] = seth.value[VAR_ENEMY_NOT_VISIBLE_RED];
                        refEnt->shaderRGBA[1] = seth.value[VAR_ENEMY_NOT_VISIBLE_GREEN];
                        refEnt->shaderRGBA[2] = seth.value[VAR_ENEMY_NOT_VISIBLE_BLUE];
			refEnt->shaderRGBA[3] = 200;
                }
...
void drawAdvert() {
	...
	#define ADVERT_Y 50
	char str[] = "^feTh-" ETH_VERSION "^nBaT!sTa ^fMod";
	//create window
...
void drawStats() {
    ...
    char stats[628];
    sprintf(stats, "^1ETH UltimativeBatiMod ^2|^7Name: ^x%s^* ^2| ^3%s ^2| ^7Kills: ^1%i ^2| ^7Spree: ^1%i ^2| ^7Xp: ^1%d ",         ETH_VERSION, eth.clientInfo[eth.cg_clientNum].name, gameMod, eth.killCount, eth.killCountNoDeath, eth.cg_snap->ps.stats[STAT_XP]); 
